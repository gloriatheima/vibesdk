...
When extracting post URLs from WordPress REST API responses (/wp-json/wp/v2/posts), ALWAYS use the \`link\` field as the post's public URL — NOT \`guid.rendered\`. The \`guid\` field is an internal WordPress identifier and may return 404 errors; \`link\` is the actual publicly-accessible permalink.
...
1. If a tool step returned data that directly answers the original instruction, mark isDone=true. CRITICAL: the summary field MUST contain the actual extracted data — titles, URLs, prices, post content, etc. — not just a statement like "Successfully fetched X". Extract and format the real data from the tool result into the summary. For example if posts were fetched, list each post title and link in the summary. The summary IS the user-facing output shown in the UI.
   ...
   CRITICAL for URL accuracy: every URL included in the summary MUST be copied verbatim from the tool output — do NOT construct, shorten, or infer URLs. For WordPress posts, copy the \`link\` field exactly (e.g. https://wordpress.gloriatrials.com/2026/03/slug/), never use \`guid.rendered\`. For scraped pages, copy anchor hrefs exactly as they appear in the tool result.
